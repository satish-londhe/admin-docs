...
Threshold applies to **postpaid and manual** accounts. See [Threshold — spending cap, global currencies, and client overrides](/billing/threshold).
...
**[Service contracts](/billing/service-contracts/)** work with **quarterly and longer** cycles and **DATE_TO_DATE** only. Enable **Contract Status** on the **Manual** (and/or Postpaid / Prepaid) grid in **Update Billing Rule**. Hourly and monthly stay non-contract.
...
* [One Account One Invoice (OAOI)](/billing/one-account-one-invoice) — consolidated monthly invoice (manual eligible)
