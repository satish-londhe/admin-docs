...
tags: ["engagement", "datamount", "admin", "ipam", "panorama", "cloudstack"]
---

# Admin workflow — provider-level setup

Everything here is configured by the **StackConsole admin only** — none of it is customer-facing. This one-time (and ongoing) infrastructure setup makes the [customer provisioning phases](/engagements/datamount/phase-0-customer-order) possible as a thin, safe self-service layer.
...
**Prev:** [Confirmed architecture](/engagements/datamount/architecture) · **Next:** [CloudStack reference patterns](/engagements/datamount/cloudstack-reference-patterns)

</div>

:::info[Reference pattern — CloudStack]

Apache CloudStack solves a structurally similar problem (ASN pools, BGP peering, hierarchical CIDR reservation, VLAN lifecycle, router service profiles, physical network definition) **natively**, because it owns its whole stack end-to-end. DataMount does not — VCD, NSX-T, and Panorama are three separate vendor systems CMP must synchronize (see [ownership model](/engagements/datamount/architecture#provisioning-ownership) and [Phase 8 — Reconciliation](/engagements/datamount/phase-8-reconciliation)). CloudStack's **object model and admin UX shape** is still the right template for each screen below. Full mapping table and screenshots: [CloudStack reference patterns](/engagements/datamount/cloudstack-reference-patterns).
...
| 4 | Register F5 BIG-IP iControl REST | Pending [F5 architecture questions](/engagements/datamount/architecture#f5--open-architecture-questions) | **Custom** |
...
![CloudStack — Add IPv4 Subnet with Parent Subnet required](/img/screenshots/datamount/cloudstack-ipv4-subnet-add.png)
...
![CloudStack — Create AS Range](/img/screenshots/datamount/cloudstack-as-range-create.png)
...
CloudStack's **Add BGP Peer** dialog (Zone → BGP Peers) is a strong template for the CMP-internal BGP Peer object created in [Phases 3–4](/engagements/datamount/phase-3-nsx-t):
...
![CloudStack — Add BGP Peer with Set reservation](/img/screenshots/datamount/cloudstack-bgp-peer-add.png)
...
![CloudStack — System Service Offering for Domain Router](/img/screenshots/datamount/cloudstack-system-service-offering.png)
...
![CloudStack — Add Physical Network](/img/screenshots/datamount/cloudstack-physical-network-add.png)
...
![CloudStack — Zone details with Dedicate Zone](/img/screenshots/datamount/cloudstack-zone-dedicate.png)

---

## 2.6 Rate card / service catalog mapping

Map technical profiles to sellable packages (customer sees these in [Phase 0](/engagements/datamount/phase-0-customer-order)):
...
| Enterprise Plus | Critical workloads | + advanced threat protection, enhanced logging, higher throughput; consider [Dedicate Zone–style](/engagements/datamount/cloudstack-reference-patterns#dedicate-zone--enterprise-tier-precedent) dedicated VRF/ASN |
...
:::note[Pending open questions]

Placeholder until [F5 architecture](/engagements/datamount/architecture#f5--open-architecture-questions) is confirmed: base pool/WAF/SSL policy templates, partition-per-tenant vs dedicated-appliance model, and whether F5 provisioning is gated behind the [BGP validation gate](/engagements/datamount/phase-5-bgp-gate) or independent.
