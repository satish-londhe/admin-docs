...
See [Snapshot & Backup — Restoring a volume snapshot](/orchestrators/cloudstack/snapshot-backup#restoring-a-volume-snapshot) for CloudStack's **Create Template from Snapshot** option (root disk only).
