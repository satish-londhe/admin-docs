...
**Prev:** [Admin setup](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396296195) · **Next:** [Provider abstraction](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396591106)
...
Full admin workflow with inline parity callouts: [Admin setup](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396296195).
...
>
> CloudStack works because it provisions its own **Virtual Router** as a System VM and manages BGP/VLAN/subnet allocation as pieces of its own orchestration. The DataMount stack is structurally different: **three separate vendor platforms** (VCD, NSX-T, Panorama), each with partial IPAM/BGP capabilities, which CMP must keep synchronized. That is exactly why [Phase 8 — Reconciliation](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396787742) exists — CloudStack does not need an equivalent because there is no second system of record.
>
...
../../static/img/screenshots/datamount/cloudstack-ipv4-subnet-reservation.png
![Add IPv4 Subnet — Set reservation to Account or Domain](../../static/img/screenshots/datamount/cloudstack-ipv4-subnet-reservation.png)

../../static/img/screenshots/datamount/cloudstack-bgp-peer-add.png
![Add BGP Peer — Set reservation and password field](../../static/img/screenshots/datamount/cloudstack-bgp-peer-add.png)

### 2. State-visible list views

CloudStack surfaces **Allocation state** and **Taken** timestamp directly in list tables (Guest VLAN, IPv4 Subnets), not only on detail pages. Apply the same pattern to CMP `resource_bindings` and IPAM inventory — makes [reconciliation](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396787742) and orphan detection easier to eyeball manually.

../../static/img/screenshots/datamount/cloudstack-guest-vlan.png
![Guest VLAN — Allocation state, Taken, Account, Domain columns](../../static/img/screenshots/datamount/cloudstack-guest-vlan.png)

### 3. Service offering as selectable package

CloudStack's **Add System Service Offering** (System VM type = Domain Router) bundles CPU, memory, HA, scaling, and storage into one named profile with Cancel/OK footer. Use the same shape for **VSYS Profiles** and **Virtual Router packages** in [Admin setup §2.4](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396296195#24-palo-alto--admin-configuration-wizard).

../../static/img/screenshots/datamount/cloudstack-system-service-offering.png
![System Service Offering — Domain Router profile template](../../static/img/screenshots/datamount/cloudstack-system-service-offering.png)
...
![Add IPv4 Subnet — Parent Subnet required before child CIDR](../../static/img/screenshots/datamount/cloudstack-ipv4-subnet-add.png)
...
![Create AS Range — Start and End AS Number](../../static/img/screenshots/datamount/cloudstack-as-range-create.png)
...
../../static/img/screenshots/datamount/cloudstack-physical-network-list.png
![Physical Network list — VLAN isolation, traffic types](../../static/img/screenshots/datamount/cloudstack-physical-network-list.png)

../../static/img/screenshots/datamount/cloudstack-physical-network-add.png
![Add Physical Network — Isolation method, VLAN/VNI, broadcast domain](../../static/img/screenshots/datamount/cloudstack-physical-network-add.png)

### Virtual Router — per-tenant T1 analog

../../static/img/screenshots/datamount/cloudstack-virtual-routers-list.png
![Virtual Routers list — one row per tenant router, state and IP visible](../../static/img/screenshots/datamount/cloudstack-virtual-routers-list.png)

../../static/img/screenshots/datamount/cloudstack-virtual-router-detail.png
![Virtual Router detail — NICs, Guest Network, Public IP, System Offering](../../static/img/screenshots/datamount/cloudstack-virtual-router-detail.png)

Recommend the same **one row per live tenant T1/Edge** list view for NSX-T inventory in [Phase 3](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396755006).

### Dedicate Zone — Enterprise tier precedent

../../static/img/screenshots/datamount/cloudstack-zone-dedicate.png
![Zone details — Dedicated flag and Dedicate Zone action](../../static/img/screenshots/datamount/cloudstack-zone-dedicate.png)
...
- [Provider abstraction — CloudStack vs VCD path](https://stackconsole.atlassian.net/wiki/spaces/DataMount/pages/396591106)
